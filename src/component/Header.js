...
//   const { token, isAuthenticated, authHeaders, logout } = useContext(AuthContext);
//   const name = localStorage.getItem("name");
//   const [showDropdown, setShowDropdown] = useState(false);
//   // const token = localStorage.getItem("token");
//   // 🔥 NEW STATE (ONLY FOR SHOP)
...
//     // ⚠️ AUTO LOGOUT IF TOKEN OR ROLE MISSING
//   useEffect(() => {
//     if (!token || !role) {
//       logout();
//       dispatch(clearCart());
//       navigate("/login", { replace: true });
//     }
//   }, [token, role, logout, dispatch, navigate]);
...
//                     className="user-info"
//                   >
//                     <strong>Hello {isAuthenticated ? "User" : "Guest"}</strong>
//                   </div>
