...
const handleVendorDetails = async (vendor) => {
  try {
    setSelectedVendor(vendor);

    const res = await axios.post(
      "http://tech-shop.techsaga.live/api/v1/vendor/details",
      { vendor_id: vendor.id },
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (res.data.status) {
      ...
      setShowVendorModal(true); // ✅ open modal
    }
  } catch (err) {
    console.error(err);
...
const handleBankDetails = async (vendor) => {
  try {
    setSelectedVendor(vendor);

    const res = await axios.post(
      "http://tech-shop.techsaga.live/api/v1/vendor/get-bank-details",
      { vendor_id: vendor.id },
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (res.data.status) {
      ...
      setShowBankModal(true); // ✅ open modal
    }
  } catch (err) {
    console.error(err);
                    ...
                    vendors.map((vendor) => (
                      <tr key={vendor.id}>
                        ...
                        <td className="text-center">
                          ...
                          <td>
                            <div className="d-flex gap-2">
                            <button className="btn_details" onClick={() => handleVendorDetails(vendor)}>Vendor Details</button>
                            <button className="bank_details" onClick={() => handleBankDetails(vendor)}>Bank Details</button>
                          </div>
                          </td>
                      </tr>
                    ))
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          <div className="modal-header">
            ...
            <button className="btn-close" onClick={() => setShowVendorModal(false)}></button>
          </div>

          <div className="modal-body">
            {vendorDetails && (
              <ul className="list-group">
                <li className="list-group-item"><b>Name:</b> {vendorDetails.name}</li>
                <li className="list-group-item"><b>Email:</b> {vendorDetails.email}</li>
                <li className="list-group-item"><b>Phone:</b> {vendorDetails.phone_number}</li>
                <li className="list-group-item"><b>Business:</b> {vendorDetails.business_name || "-"}</li>
                <li className="list-group-item"><b>GSTIN:</b> {vendorDetails.gstin_no || "-"}</li>
              </ul>
            )}
          </div>
        </div>
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          ...
          <div className="modal-body">
            {bankDetails && (
              <ul className="list-group">
                <li className="list-group-item"><b>Vendor Name:</b> {bankDetails.vendor_name}</li>
                <li className="list-group-item"><b>Account Holder:</b> {bankDetails.account_holder_name}</li>
                <li className="list-group-item"><b>Bank Name:</b> {bankDetails.bank_name}</li>
                <li className="list-group-item"><b>Account No:</b> {bankDetails.account_number}</li>
                <li className="list-group-item"><b>IFSC:</b> {bankDetails.ifsc_code}</li>
              </ul>
            )}
          </div>
        </div>
