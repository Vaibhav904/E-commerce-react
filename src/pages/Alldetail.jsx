...
//       console.log('nefiouuberiovb-fveihbverih v',productData.variants?.[0]?.stock ) ;
...
export default function Alldetail() {
  ...
  /* ✅ FIX: quantity adjust when variant changes */
  useEffect(() => {
    if (stock === 0) {
      setQuantity(1);
  ...
  const handleAddToCart = async () => {
    if (stock === 0) return; // ✅ block if no stock
    ...
    <div className="container my-4">
      <div className="row gx-5">
        ...
        <div className="col-md-6">
          ...
          {stock === 0 && (
            <p className="text-danger fw-bold mt-2">Out of Stock</p>
          )}
          ...
          <button
            className={`btn-shop-pay mt-3 ${
              isWishlisted ? "btn-dark" : ""
            }`}
            onClick={handleWishlistToggle}
          >
            {isWishlisted
