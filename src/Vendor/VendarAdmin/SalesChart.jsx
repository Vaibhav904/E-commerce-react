...
useEffect(() => {
  ...
  axios
    .get("http://tech-shop.techsaga.live/api/v1/vendor/total-sales", {
      headers: {
    ...
    .then((res) => {
      if (res.data.status) {
        setTotalSales(res.data.total.formatted); // ✅ correct path
        setSalesComparison(res.data.message); // optional
      }
    })
...
useEffect(() => {
  ...
  axios
    ...
    .then((res) => {
      if (res.data.status) {
        setTotalOrders(res.data.formatted_orders); // ✅ correct
        setOrdersComparison(res.data.message); // optional
      }
    })
    .catch((err) => console.log(err));
}, []);
  ...
  axios
    ...
    .then((res) => {
      if (res.data.status) {
        ...
        // safer conversion
        const sales = res.data.data.sales.map((item) =>
          parseFloat(item)
        );
