...
const addVariant = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  for (let i = 0; i < variants.length; i++) {
    ...
    formData.append(`variants[${i}][base_price]`, Number(v.base_price));
    formData.append(`variants[${i}][discount]`, Number(v.discount));
    formData.append(`variants[${i}][stock]`, Number(v.stock));
  ...
  try {
    await axios.post(
      "http://tech-shop.techsaga.live/api/v1/vendor/products/store",
      formData,
      {
        headers: {
          Authorization: `Bearer ${vendorToken}`,
